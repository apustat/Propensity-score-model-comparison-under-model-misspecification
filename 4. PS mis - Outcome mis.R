...
for (i  in 1:r) {
  ...
  # Force misspecification: remove X8, X9 if they are in the selection
  misspec_vars_ps <- setdiff(colnames(X), c("X8", "X9")) #Removing X8 X9 indicates failing to consider observed/unobserved confounders
  
  # Build formula for logistic regression
  ps_formula <- as.formula(
    paste("A ~", paste(misspec_vars_ps, collapse = " + "), "+ X1:X2 + X1:X3")) #adding two interactions
  ...
  rf_data <- dat[, c("A", misspec_vars_ps)]
  ...
  # AIPW estimator
  # Fit outcome model
  # Force misspecification: remove X6, X7 if they are in the selection
  misspec_vars_out <- setdiff(colnames(X), c("X6", "X7"))
  outcome_vars <- union("A", misspec_vars_out) #adding treatment variable in the outcome model if it's not selected by any chance
...
colnames(result)<-c("IPW-LR", "IPW-RF", "IPW-LDA", "IPW-SVM",
                    ...
                    "RSM")
apply(result,2,mean)
write.csv(result, file="C:/Users/apust/OneDrive - University of Nebraska Medical Center/Desktop/PS comparison/4.n=200-rho=0.7.csv")
